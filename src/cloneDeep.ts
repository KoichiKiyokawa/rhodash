...
export function cloneDeep<T>(value: T): T {
  ...
  if (value instanceof Map)
    return new Map(Array.from(value.entries()).map(([k, v]) => [k, cloneDeep(v)])) as unknown as T
  if (value instanceof Set)
    return new Set(Array.from(value.entries()).map(([k, v]) => [k, cloneDeep(v)])) as unknown as T
